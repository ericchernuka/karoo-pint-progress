...
- [`docs/TEST_BOUNDARY.md`](docs/TEST_BOUNDARY.md) defines the local verification boundary and
  the behavior that still needs an on-device Karoo smoke test.
...
and rerun it to regenerate them. Keep light and night colors in their resource qualifiers
...
Karoo caller gate. Preserve the existing caller authorization and CI’s read-only, no-secrets
boundary described in [`README.md`](README.md).
...
Preserve the one-Hz view-update limit and cancellation behavior. Consume all six `ViewConfig`
inputs—`gridSize`, `viewSize`, `textSize`, `alignment`, `boundariesEnabled`, and `preview`—and
retain the on-device Karoo smoke-test requirement in [`README.md`](README.md) and
