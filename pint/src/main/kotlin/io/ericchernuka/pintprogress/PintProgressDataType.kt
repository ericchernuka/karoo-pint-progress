...
class PintProgressDataType(
    ...
    override fun startView(context: Context, config: ViewConfig, emitter: ViewEmitter) {
        ...
        if (config.preview) {
            emitter.updateView(renderer.render(PintPresentation.displayFor(PintViewReducer.previewFrame())))
            return
        }
        ...
        val job = CoroutineScope(Dispatchers.Default).launch {
            ...
            suspend fun render(frame: PintFrame) {
                ...
                emitter.updateView(renderer.render(PintPresentation.displayFor(frame)))
