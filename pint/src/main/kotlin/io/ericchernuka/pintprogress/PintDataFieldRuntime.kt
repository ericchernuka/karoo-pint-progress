...
internal class PintDataFieldRuntime(private val nowMillis: () -> Long, private val waitMillis: suspend (Long) -> Unit,
) { suspend fun runNumericStream(calorieStates: Flow<StreamState>, caloriesPerBeer: Flow<Int>, dataTypeId: String, emit: suspend (StreamState) -> Unit, ) { var lastEmissionMillis: Long? = null
        calorieStates
    ...
    suspend fun runNumericPreview(emit: suspend (String) -> Unit) { while (true) { PintTextStreamState.previewMessages().forEach { message ->
                emit(message)
                waitMillis(VIEW_UPDATE_INTERVAL_MILLIS) } } }
    suspend fun runGraphicalStream(calorieStates: Flow<StreamState>, caloriesPerBeer: Flow<Int>, emit: suspend (PintFrame) -> Unit, ) { val reducer = PintViewReducer()
        ...
        calorieStates
            ...
            .collect { (state, target) ->
                reducer.accept(state, target)?.frames?.forEach { timedFrame ->
                    ...
                    lastEmissionMillis = nowMillis() } } }
    suspend fun runGraphicalPreview(emit: suspend (PintFrame) -> Unit) { while (true) { PintViewReducer.previewFrames().forEach { frame ->
                emit(frame)
                waitMillis(VIEW_UPDATE_INTERVAL_MILLIS) } } }
    ...
    private companion object { const val VIEW_UPDATE_INTERVAL_MILLIS = 1_000L } }
