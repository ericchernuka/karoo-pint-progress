    ...
    companion object {
        inline fun <reified T> create(packageName: String, handler: IHandler): Emitter<T> {
            return object : Emitter<T> {
                private val cancellation = CancellationHandle()
...
/** Makes cancellation safe when the host stops a session before it finishes starting. */
@PublishedApi
internal class CancellationHandle {
    private val lock = Any()
    private var cancellable: (() -> Unit)? = null
    private var cancelled = false

    fun set(cancellable: () -> Unit) {
        ...
        if (cancelImmediately) cancellable()
    }
